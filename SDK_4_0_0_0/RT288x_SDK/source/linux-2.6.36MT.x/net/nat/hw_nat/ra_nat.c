...
/* push different VID for WiFi pseudo interface or PCI external NIC */
...
#if defined (CONFIG_RALINK_RT6855) || defined (CONFIG_RALINK_RT63365) || defined (CONFIG_RALINK_RT6352)
...
static void PpeSetInfoBlk2(struct _info_blk2 *iblk2, uint32_t fpidx, uint32_t port_mg, uint32_t port_ag)
{
	...
	reg = RegRead(RALINK_FRAME_ENGINE_BASE + 0x2704);
	reg |= (0x1 << 11);
	RegWrite(RALINK_FRAME_ENGINE_BASE + 0x2704, reg);
#endif
		...
		if ((foe_entry->ipv4_hnapt.vlan1 & VLAN_VID_MASK) == LAN_PORT_VLAN_ID) {
			if ((bind_dir == DOWNSTREAM_ONLY) || (bind_dir == BIDIRECTION)) {
				if (IS_IPV4_GRP(foe_entry)) {
				    ...
				    PpeSetInfoBlk2(&foe_entry->ipv6_3t_route.iblk2, 8, 0x3F, 2);
				}
#endif
			} else {
				return 1;
			...
			if ((bind_dir == UPSTREAM_ONLY) || (bind_dir == BIDIRECTION)) {
				if (IS_IPV4_GRP(foe_entry)) {
					...
					PpeSetInfoBlk2(&foe_entry->ipv6_3t_route.iblk2, 8, 0x3F, 1);
				}
#endif
...
#else
		/*  RT3052, RT335x */
		if ((foe_entry->ipv4_hnapt.vlan1 & VLAN_VID_MASK) ==
		    LAN_PORT_VLAN_ID) {
			if ((bind_dir == DOWNSTREAM_ONLY)
			    || (bind_dir == BIDIRECTION)) {
