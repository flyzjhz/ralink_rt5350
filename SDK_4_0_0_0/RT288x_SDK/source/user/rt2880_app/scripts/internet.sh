#!/bin/sh
#
# $Id: internet.sh,v 1.139.2.6 2012-02-21 11:34:36 chhung Exp $
#
# usage: internet.sh
#
...
if [ "$opmode" = "0" ]; then
	if [ "$CONFIG_RAETH_ROUTER" = "y" -a "$CONFIG_LAN_WAN_SUPPORT" = "y" ]; then
		...
		if [ "$CONFIG_P5_RGMII_TO_MAC_MODE" = "y" ]; then
			...
			if [ "$CONFIG_RALINK_RT6855" = "y" -o "$CONFIG_RALINK_RT63365" = "y" -o "$CONFIG_RALINK_RT6352" = "y" ]; then
				config-vlan.sh 3 0
...
# RTDEV_MII support: start mii iNIC after network interface is working
	...
	if [ "$CONFIG_RT_3052_ESW" = "y" -a "$CONFIG_LAN_WAN_SUPPORT" = "y" ]; then
		if [ "$CONFIG_P5_RGMII_TO_MAC_MODE" = "y" ]; then
			echo "##### restore Ralink ESW to dump switch #####"
			if [ "$CONFIG_RALINK_RT6855" = "y" -o "$CONFIG_RALINK_RT63365" -o "$CONFIG_RALINK_RT6352" = "y" ]; then
				config-vlan.sh 3 0 
			elif [ "$CONFIG_RALINK_RT71100" = "y" ]; then
				config-vlan.sh 4 0
			else
